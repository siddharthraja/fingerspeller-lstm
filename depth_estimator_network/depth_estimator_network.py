...
class BatchNormalizer(object):
    ...
    def __init__(self, depth, epsilon, ewma_trainer, output_scale):
    ...
    def normalize(self, x, train=True):
        if train:
            ...
            with tf.control_dependencies([assign_mean, assign_variance]):
                return tf.nn.batch_norm_with_global_normalization( x, mean, variance, self.beta, self.gamma, self.epsilon, self.output_scale)
            ...
            return tf.nn.batch_norm_with_global_normalization(x, mean, variance, local_beta, local_gamma, self.epsilon, self.output_scale)


filenames = '' #TODO: get depth image file names for labels
color_files = ''
depth_files = ''
...
def depthnet(_tensors):
    # basic-ass residual network for depth estimation

    # TODO try it with different filter depths for fun
...
def concat_images(imga, imgb):
    """
    Combines a depth image and a color image ndarrays side-by-side.
    """
    ha, wa = imga.shape[:2]
    hb, wb = imgb.shape[:2]
...
#TODO insertion point for depth switch
...
tf.image_summary("depthimage", depthimage, max_images=1)
tf.image_summary("pred_depth", pred_rgb, max_images=1)
tf.image_summary("colorimage", grayscale_rgb, max_images=1)
...
try:
    while not coord.should_stop():
        ...
        if step % 100000 == 99998:
            save_path = saver.save(sess, "model_%d.ckpt"%step)
